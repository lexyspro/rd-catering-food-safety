...
export const metadata: Metadata = { title: "Equipment Cleaning" };

export default async function EquipmentCleaningPage() {
  const session = await auth();
  const role = session?.user?.role as string;
  const userId = session?.user?.id as string;

  const records = await prisma.equipmentCleaningRecord.findMany({
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-title">Equipment Cleaning</h1>
          <p className="page-desc">Log and review equipment cleaning records</p>
        </div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.8fr", gap: 24, alignItems: "start" }}>
        <CleaningForm type="equipment" />
        ...
        <div className="card">
          ...
          {records.length === 0 ? (
            <div className="empty-state">
              ...
              <div className="empty-state-desc">Use the form to log today's cleaning</div>
            </div>
          ) : (
                ...
                <tbody>
                  {records.map((r) => (
                    <tr key={r.id}>
                      ...
                      {role !== "STAFF" && (
                        <td>
                          {r.status === "PENDING" && <SignOffButton id={r.id} type="equipment" />}
                        </td>
                      )}
                    </tr>
                  ))}
